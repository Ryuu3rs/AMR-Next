...
`VITE_COMMUNITY_API_ORIGIN` repository variable. `.env.example` also documents the optional `VITE_METADATA_API_ORIGIN` (adds the self-hosted metadata catalog as a host permission when set; leave blank to fall back to AniList directly).
