...
## Re-verification: Bato.to, 2026-07-17

~30 titles in a real user's library were stuck on bato.to as their source (dead per the
row above). Re-checked live before finalizing the "don't add" decision, since the
2026-06-14 row alone is a month-old automated probe:

- `bato.to` - connection failure (000). Consistent with the site's known 2025 shutdown.
- `bato.si`, `bato.ac`, `bato.bz`, `bato.cc`, `bato.cx`, `bato.day`, `mto.to`, `dto.to`,
  `battwo.com` - HTTP 200 but 6-76 byte stub bodies, not real content.
- `batotoo.com`, `comiko.net` - GoDaddy domain-parking pages.
- `batocomic.com`, `batocomic.net`, `zbato.com`, `readbato.com` - "Redirecting..." pages
  whose only outbound link is `router.parklogic.com` (a parking-domain monetizer).
- `mangatoto.com` - a live but unrelated Thai-language site squatting the old mirror
  domain; not the bato v3 engine.

Checked via plain HTTP only (this codebase's adapters are fetch-based - see
`apps/extension/src/sources.ts` - tab-render is a chapter-capture fallback only, not a
listing/search path, so a JS-rendered successor still couldn't be scraped this way
regardless). If you know of a live, fetch-servable bato successor, say so before this
gets re-litigated - otherwise treat the source as gone, not merely gated.
